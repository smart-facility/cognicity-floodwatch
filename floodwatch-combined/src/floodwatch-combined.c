...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  if(area_tuple == NULL) {
    ...
    APP_LOG(APP_LOG_LEVEL_DEBUG, "%s",area_temp);
    ...
    while( area_temp != NULL )
    {
      ...
      token_count++;
      APP_LOG(APP_LOG_LEVEL_DEBUG, "%s",flood_info[token_count].area);
    ...
    {
      flood_info[token_count].source= source_temp;
      APP_LOG(APP_LOG_LEVEL_DEBUG, "%s",flood_info[token_count].source);
      token_count++;
